...
function DayWaterForm(){  
    ...
    useEffect(()=>{
        try{ 
            ...
            let startTime = changeMomentType(moment,'YYYY-MM-DD HH').subtract(2,'day').format('YYYY-MM-DD') +afterFix;
    ...
    const onSearchTable = (e,initData = null)=>{
        ...
        setIsLoading(true)
        getHourFlowForm(paramsObj).then(res=>{
            // console.log("getHourFlowForm返回的数据是",res)
            setIsLoading(false)
 
            // 判断是否为空数据
            if(!res || !res['data1:'] || !res['data1:'].data || !res['data1:'].data.length){
            ...
            let t_dataResource = Tools.handleHourFlowTableDataY(typeList,res,31,10,rowTitleArr )  
    ...
    const handleOutput = (mode)=>{ 
        ...
        const excelFileName = `山口岩水库分日水位明细报表(${searchTxtObj.startTime.slice(0,4)})`;
        ...
        const appendBottom = '水位单位：米,库容单位：万立方米, 入库/出库流量单位：立方米/秒'+',查询时间：'+searchTxtObj.startTime.slice(0,4)+'年'+searchTxtObj.startTime.slice(5,7)+'月'+searchTxtObj.startTime.slice(8,10)+'日'
        if(mode == 'excel'){ 
