...
import { getRainChangeDay,getRainChangeWeek,getRainChangeMonth } from 'Services/Home';
import { changeDataLine2,reverseFormatDate,
        ...
        switch(radioItem){
            case 'day':
                ...
                dataSource.map(item=>{ 
                    if(isNotArriveDay){ 
                        turnsRow ++;
                        if(item.tm == goalCurrentDayTime || item.tm == '*' + goalCurrentDayTime || item.tm == '合计雨量' ){
            ...
            case 'week': 
                ...
                dataSource.map(item=>{ 
                    if(isNotArriveWeek){ 
                        turnsRow ++;
                        if(item.tm == goalCurrentWeekTime || item.tm == '*' + goalCurrentWeekTime|| item.tm == '合计雨量' ){
            ...
            case 'month':
                ...
                dataSource.map(item=>{ 
                    if(isNotArriveMonth){ 
                        turnsRow ++;
                        if(item.tm == goalCurrentMonthTime || item.tm == '*' + goalCurrentMonthTime|| item.tm == '合计雨量' ){
    ...
    const drawContentRightTop = async(t_echartList ) =>{  
        ...
        var option = {
            ...
            legend: {
                top:5,
                left:5,
                data:[typeList[0].stnm,typeList[1].stnm,typeList[2].stnm,typeList[3].stnm,typeList[4].stnm]
            },
            grid: {
                top: 70,
                bottom: 50
        ...
        <div className='rainChange-div homeTable-div commTable-div'> 
            ...
            <div className='body-bottom-div'>  
                ...
                <div className='bottom-bottom' style={{'visibility':isTableCollapse ? 'hidden' : 'visible' , 'opacity':0}} >  
                    ...
                    <div id='bottom-echart-top' style={{width:'600px',height:'300px','opacity':isShowEchart ?'1' : '0' }} ref={echartTopNodeRef}></div>
                    <div id='bottom-echart-middle' style={{width:'600px',height:'300px','opacity':isShowEchart ?'1' : '0' }} ref={echartMiddleNodeRef}></div>
                    <div id='bottom-echart-bottom' style={{width:'600px',height:'300px','opacity':isShowEchart ?'1' : '0' }} ref={echartBottomNodeRef}></div> 
                </div>    
            </div>
        </div>
    )
