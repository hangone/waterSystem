  ...
  const getDayData = async (isInit = false) => {
    try {
      ...
      dayCap1.forEach((item, index) => {
        ...
        t_item =
          prefixDate + columnsTimeList[timeTurns] + ':' + curMinute + ':00'
        t_res.push({
      ...
      t_res.map((item, index) => {
        if (isNotArriveDay) {
          ...
          if (
            index != 0 &&
            index != t_res.length - 1 &&
            verifyMinute(item.tm, goalCurrentDayTime, '<=') &&
      ...
      if (len) {
        ...
        let b1 = document.querySelector(
          '.flow-flag-div>div:nth-child(5) .inner-txt span.inner'
        )
        setDomInnerText(b1, drp1)
        // 老庵里
        let drp0 = changeDataLine2(dataObj[typeList2[1].stcd + 'drp'])
        let b0 = document.querySelector(
          '.flow-flag-div>div:nth-child(3) .inner-txt span.inner'
        )
        setDomInnerText(b0, drp0)
        // 新泉站
        let drp2 = changeDataLine2(dataObj[typeList2[2].stcd + 'drp'])
        let b2 = document.querySelector(
          '.flow-flag-div>div:nth-child(4) .inner-txt span.inner'
        )
        setDomInnerText(b2, drp2)
        // 西江口
        let drp3 = changeDataLine2(dataObj[typeList2[3].stcd + 'drp'])
        let b3 = document.querySelector(
          '.flow-flag-div>div:nth-child(2) .inner-txt span.inner'
        )
        setDomInnerText(b3, drp3)
        // 张佳坊
        let drp4 = changeDataLine2(dataObj[typeList2[4].stcd + 'drp'])
        let b4 = document.querySelector(
          '.flow-flag-div>div:nth-child(1) .inner-txt span.inner'
        )
