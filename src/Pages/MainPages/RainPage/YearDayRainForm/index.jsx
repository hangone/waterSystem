...
import { DatePicker, Button,Empty ,Table,Menu, Dropdown  } from 'antd';
import moment from 'moment'
import { SearchOutlined,DownloadOutlined,PrinterOutlined } from '@ant-design/icons';
import { getYearDayRainForm } from 'Services/Home/search';
...
function DayRainForm(){  
    ...
    const [activeMenuName,setActiveMenuName] = useState('新泉站')
    const [activeMenuStcd,setActiveMenuStcd] = useState('')
    // 列数据（不含顶部栏的表头）
    const rowTitleArr  =  [
        {month:'1日'},
        {month:'2日'},
    ...
    useEffect(()=>{
        try{ 
            ...
            // 设置下拉菜单项
            let t_activeMenuStcd = ''
            let t_activeMenuName = ''
            setMenu(typeList); 
            typeList.forEach(item=>{ 
                if(item.stnm == activeMenuName){ 
                    setActiveMenuStcd(item.stcd);
                    t_activeMenuStcd = item.stcd
                    t_activeMenuName = activeMenuName
                }
            }) 
    ...
    const onSearchTable = (initData = null)=>{
        ...
        setIsLoading(true)
        getYearDayRainForm(paramsObj).then(res=>{
            // console.log("getYearDayRainForm返回的数据是",res,res.data) 
            // 空值检测
            ...
            let simbleStation = paramsObj.stcd;
            let t_dataResource = Tools.handleYearDayRainTableDataY(typeList,res.data,31,10,rowTitleArr,prefix,simbleStation)  
    ...
    const handleOutput = (mode)=>{ 
        ...
        const excelFileName = `山口岩水库全年逐日降雨明细表(${activeMenuName}、${searchTxtObj.time.slice(0,4)} )`;

        // 获取行列表头数据    
        // let rowArr = ['月份','1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月']
        // let colArr = ['月份','王狗冲','老庵里','新泉站（雨量）','西江口','张佳坊','新泉站（雨量）','新泉站','厂房']
        ...
        const cLen = colArr.length;
        const rowMergesArr =  [
            // {s: {r: rLen - 4, c: 1}, e: {r: rLen - 4, c: cLen-1}},  
            {s: {r: rLen - 3, c: 1}, e: {r: rLen - 3, c: cLen-1}},  
            {s: {r: rLen - 2, c: 1}, e: {r: rLen - 2, c: cLen-1}},  
            {s: {r: rLen - 1, c: 1}, e: {r: rLen - 1, c: cLen-1}},   
            {s: {r: rLen, c: 1}, e: {r: rLen, c: cLen-1}}, 
        ]
        const appendBottom = '降雨量单位：毫米,'+'查询站点：'+activeMenuName+',查询年份：'+searchTxtObj.time.slice(0,4)+'年'
        if(mode == 'excel'){ 
    ...
    const onSelectDropdown = (stcd)=>{ 
         ...
         typeList.forEach(item=>{ 
             if(item.stcd == stcd){ 
                 setActiveMenuStcd(item.stcd);
                 setActiveMenuName(item.stnm);
                 t_activeMenuStcd = item.stcd
                 t_activeMenuName = item.stnm
             }
         }) 
         setMenu(typeList); 
        ...
        <div className='dayRainChart-div commTable-div'>
            <div className='body-top-div'>
                            ...
                            <Menu>
                                {
                                    ...
                                    ?menu.map(item=>{
                                        return (     
                                            <Menu.Item onClick={()=>onSelectDropdown(item.stcd)}>
                                                <div > {item.stnm} </div>
                                            </Menu.Item> 
                                        )
                                    })
                ...
                <div className='top-right'>  
                    <div className='top-title'>全年逐日降雨明细报表</div>
