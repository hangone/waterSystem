...
import { getCapChangeDay,getInflowChangeDay,getOutflowChangeDay } from 'Services/Home'; 
import { changeDataLine,stripDayUselessData,
...
function WaterChange(){    
    ...
    const getDayData = async (isInit=false) =>{ 
        try{
            ...
            datetimeList = datetimeList.slice(0,res.length)
            ...
            datetimeList.forEach((item,index)=>{ 
                ...
                t_res.push( { 
                    ...
                    [typeList[0].stcd+'w']:changeDataLine(dayData1[item] ), 
                    [typeList[1].stcd+'inflow']:changeDataLine(dayData2[item] ),
                    [typeList[2].stcd+'outflow']:changeDataLine(dayData3[item]), 
                }) 
                // 图表数据
                t_echartsX1.push((index  == nowTime ? '*' : '' )+reverseFormatDate2(item).slice(11) )
                t_echartsX2.push((index  == nowTime ? '*' : '' )+reverseFormatDate2(item).slice(11) )
                t_echartsX3.push((index  == nowTime ? '*' : '' )+reverseFormatDate2(item).slice(11) )

                t_subData0.push(changeDataLine(dayData1[item] )) 
                t_subData1.push(changeDataLine(dayData2[item] ))
                t_subData2.push(changeDataLine(dayData3[item] )) 

                // 计算最大值和最小值
                if(dayData1[item] || dayData1[item] == 0){
                    if(dayData1[item] > t_max1){
                        t_max1 = dayData1[item]
                    }
                    if(dayData1[item] < t_min1){
                        t_min1 = dayData1[item] 
                    }
                }
                if(dayData2[item] || dayData2[item] == 0){
                    if(dayData2[item] > t_max2){
                        t_max2 = dayData2[item]
                    }
                    if( dayData2[item] < t_min2){
                        t_min2 = dayData2[item] 
                    } 
                }
                if(dayData3[item] || dayData3[item] == 0){
                    if(dayData3[item] > t_max3){
                        t_max3 = dayData3[item]
                    }
                    if(dayData3[item] < t_min3){
                        t_min3 = dayData3[item] 
                    } 
                } 
            })  
            setDataSource(t_res)   
        ...
        <div className='waterChange-div homeTable-div commTable-div'> 
            ...
            <div className='body-bottom-div' style={{'background':'white'}}>  
                ...
                <div className='bottom-bottom'  style={{'visibility':isTableCollapse ? 'hidden' : 'visible','opacity':0}} >   
                    ...
                    <div id='bottom-echart-top' style={{width:'600px',height:'220px'}} ref={echartTopNodeRef}></div>
                    <div id='bottom-echart-middle' style={{width:'600px',height:'220px' }} ref={echartMiddleNodeRef}></div>
                    <div id='bottom-echart-bottom' style={{width:'600px',height:'220px' }} ref={echartBottomNodeRef}></div>            
                 </div>     
            </div>
        </div>
    )
